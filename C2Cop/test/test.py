...
def check_content(out_f, ref_f):
    out_lines = [l.strip() for l in out_f.readlines() if l.strip() is not ""]
    ref_lines = [l.strip() for l in ref_f.readlines() if l.strip() is not ""]
...
def check_mem(mem_f):
    mem_lines = [l.strip() for l in mem_f.readlines() if l.strip() is not ""]
